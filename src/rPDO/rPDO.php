 ...
 *
 * rPDO обеспечивает централизованный доступ к данным через простой объектно-ориентированный API к
 ...
 * класс {@link rPDOObject} (основной или пользовательский), загружающий менеджеры источников данных
 ...
 * и карты метаданных для rPDO, поддерживающие классы, модели и свойства
...
class rPDO {
    ...
    /**
     * @var \PDO Ссылка на экземпляр PDO, используемый текущим rPDOConnection.
     */
    public $pdo= null;
    /**
     * @var array Параметры конфигурации для экземпляра rPDO.
     */
    public $config= null;
    /**
     * @var Om\rPDODriver Экземпляр rPDODriver для использования экземплярами rPDOConnection.
     */
    public $driver= null;
    /**
     * @var rPDOMap Карта метаданных источника данных для всех загруженных классов.
     */
    public $map= null;
    /**
    ...
    /**
     * @var Om\rPDOManager Менеджер источников данных по запросу.
     */
    public $manager= null;
    /**
     * @var Cache\rPDOCacheManager Поставщик услуг кэширования зарегистрировался для этого экземпляра.
     */
     ...
     * классы для этого экземпляра rPDO.
     */
    ...
    /**
     * @var rPDOConnection Текущее rPDOConnection для этого экземпляра rPDO.
     */
    public $connection = null;
    /**
     * @var array Соединения PDO, управляемые этим экземпляром rPDO.
     */
    private $_connections = array();
    /**
     * @var integer Уровень ведения журнала для экземпляра rPDO.
     */
    protected $logLevel= self::LOG_LEVEL_FATAL;
    /**
     * @var string Цель ведения журнала по умолчанию для экземпляра rPDO.
     */
    ...
    /**
     * Create, retrieve, or update specific rPDO instances.
     *
     ...
     * with the provided $id already exists in rPDO::$instances.
     *
     * @throws rPDOException If a valid instance is not retrieved.
     * @return rPDO An instance of rPDO.
     */
    public static function getInstance($id = null, $config = null, $forceNew = false) {
        ...
        if ($forceNew || !array_key_exists($id, $instances) || !($instances[$id] instanceof rPDO)) {
            $instances[$id] = new rPDO(null, null, null, $config);
        } elseif ($instances[$id] instanceof rPDO && is_array($config)) {
            $instances[$id]->config = array_merge($instances[$id]->config, $config);
        }
        if (!($instances[$id] instanceof rPDO)) {
            throw new rPDOException("Error getting " . __CLASS__ . " instance, id = {$id}");
        }
        return $instances[$id];
    }
     ...
     *
     * @return ClassLoader Экземпляр автозагрузчика, используемый всеми экземплярами rPDO.
     */
     ...
     * Конструктор rPDO.
     *
     * Этот метод используется для создания нового объекта rPDO с подключением к определенному
     ...
     * @param array|ContainerInterface $options       Контейнер зависимостей или массив параметров rPDO.
     *                                                Вы должны настроить массив параметров rPDO внутри контейнера и передать контейнер для
     *                                                будущая совместимость.
     * @param array|null               $driverOptions Параметры PDO, зависящие от конкретного драйвера.
     *
     * @throws rPDOException Если возникает ошибка при создании экземпляра.
     */
    public function __construct($dsn, $username= '', $password= '', $options= array(), $driverOptions= null) {
        try {
            ...
            if ($this->services === null) {
                $this->services = new rPDOContainer();
            }
            $this->setLogLevel($this->getOption('log_level', null, rPDO::LOG_LEVEL_FATAL, true));
            ...
            if (isset($this->config[rPDO::OPT_CONNECTIONS])) {
                $connections = $this->config[rPDO::OPT_CONNECTIONS];
            ...
            $initOptions = $this->getOption(rPDO::OPT_CONN_INIT, null, array());
            ...
            if (isset($this->config[rPDO::OPT_BASE_CLASSES])) {
                foreach (array_keys($this->config[rPDO::OPT_BASE_CLASSES]) as $baseClass) {
                    $this->loadClass($baseClass);
                }
            }
            if (isset($this->config[rPDO::OPT_CACHE_PATH])) {
                $this->cachePath = $this->config[rPDO::OPT_CACHE_PATH];
            }
        } catch (Exception $e) {
            throw new rPDOException("Не удалось создать экземпляр rPDO: " . $e->getMessage());
        }
    }

    /**
     * Инициализируйте конфигурационный массив rPDO.
     *
     * @param ContainerInterface|array $data Источник входных данных конфигурации. В настоящее время принимает зависимость
     * контейнер, содержащий запись 'config', содержащую массив конфигурации rPDO, или массив
     * содержащий конфигурацию напрямую (устарел).
     *
     * @return array Массив конфигурационных данных rPDO.
     */
    protected function initConfig($data) {
        ...
        if (!is_array($data)) {
            $data = array(rPDO::OPT_TABLE_PREFIX => '');
        }
    ...
    /**
     * Добавьте экземпляр rPDOConnection в пул подключений rPDO.
     *
    ...
    public function addConnection($dsn, $username= '', $password= '', array $options= array(), $driverOptions= null) {
        $added = false;
        $connection= new rPDOConnection($this, $dsn, $username, $password, $options, $driverOptions);
        if ($connection instanceof rPDOConnection) {
    ...
    /**
     * Получите rPDOConnection из пула подключений rPDO.
     *
     * @param array $options Множество вариантов для установления соединения.
     * @return rPDOConnection|null Экземпляр rPDOConnection или null, если не удалось восстановить соединение.
     */
    public function getConnection(array $options = array()) {
        $conn =& $this->connection;
        $mutable = $this->getOption(rPDO::OPT_CONN_MUTABLE, $options, null);
        if (!($conn instanceof rPDOConnection) || ($mutable !== null && (($mutable == true && !$conn->isMutable()) || ($mutable == false && $conn->isMutable())))) {
            if (!empty($this->_connections)) {
                ...
                $this->log(rPDO::LOG_LEVEL_ERROR, "Не удалось получить действительное rPDOConnection", '', __METHOD__, __FILE__, __LINE__);
            }
        }
    ...
    public function connect($driverOptions= array (), array $options= array()) {
        ...
        if ($this->connection instanceof rPDOConnection) {
     ...
     * добавлен в начало каждого класса rPDOObject, на который ссылаются в
     * методах rPDO, таких как {@link rPDO::loadClass()}, {@link rPDO::GetObject()},
     * {@* {@ссылка rPDO::getCollection()}, {@link rPDOObject::getOne()}, {@link
     * rPDOObject::addOne()} и т.д.
     *
     * @param string $pkg A package name to use when looking up classes in rPDO.
    ...
    public function setPackage($pkg= '', $path= '', $prefix= null, $namespacePrefix= null) {
        ...
        if ($set && is_string($prefix)) $this->config[rPDO::OPT_TABLE_PREFIX]= $prefix;
        return $set;
    }
     ...
     *
     * @param string $pkg A package name to use when looking up classes/maps in rPDO.
     * @param string $path The root path for looking up classes in this package.
     * @param string|null $prefix Provide a string to define a package-specific table_prefix.
     * @param string|null $namespacePrefix An optional namespace prefix for working with PSR-4.
     * @return bool
    ...
    public function addPackage($pkg= '', $path= '', $prefix= null, $namespacePrefix= null) {
        ...
        if (is_string($pkg) && !empty($pkg)) {
            ...
            if (!is_dir($path)) {
                ...
                $prefix= !is_string($prefix) ? $this->config[rPDO::OPT_TABLE_PREFIX] : $prefix;
            ...
            $this->log(rPDO::LOG_LEVEL_ERROR, 'addPackage called with an invalid package name.');
        }
        return $added;
    }

    /**
     * Adds metadata information about a package and loads the rPDO::$classMap.
     *
     * @param string $pkg A package name to use when looking up classes/maps in rPDO.
     * @param string $path The root path for looking up classes in this package.
     * @param string|null $namespacePrefix An optional namespace prefix for working with PSR-4.
    ...
    public function setPackageMeta($pkg, $path = '', $namespacePrefix= null) {
        ...
        if (is_string($pkg) && !empty($pkg)) {
            ...
            if (file_exists($mapFile)) {
                ...
                $this->log(rPDO::LOG_LEVEL_WARN, "Could not load package metadata for package {$pkg}. Upgrade your model.");
            }
        } else {
            $this->log(rPDO::LOG_LEVEL_ERROR, 'setPackageMeta called with an invalid package name.');
        }
        return $set;
    }
     ...
     *
     * The specified className must be rPDOObject or a derivative class.
     *
    ...
    public function loadClass($fqn, $path= '', $ignorePkg= false, $transient= false) {
        if (empty($fqn)) {
            $this->log(rPDO::LOG_LEVEL_ERROR, "No class specified for loadClass");
            return false;
        }
        $pos= strrpos($fqn, '.');
        ...
        if ($class === false) {
            $this->log(rPDO::LOG_LEVEL_ERROR, "Could not load class: {$classname} from {$fqn}");
        }
        return $class;
    }
    ...
    protected function _loadClass($class, $fqn, $included= false, $path= '', $transient= false) {
        ...
        if (!$included) {
            ...
            if (!$rt= include_once ($path . $fqcn)) {
                $this->log(rPDO::LOG_LEVEL_WARN, "Could not load class: {$class} from {$path}{$fqcn}");
                $class= false;
            }
        }
    ...
    /**
     * Get an rPDO configuration option value by key.
     *
     * @param string $key The option key.
     * @param array|null $options A set of options to override those from rPDO.
    ...
    /**
     * Sets an rPDO configuration option value.
     *
     * @param string $key The option key.
     ...
     * want to use on classes not derived from rPDOObject.
    ...
    public function call($class, $method, array $args = array(), $transient = false) {
        ...
        if (!empty($callback) && is_callable($callback)) {
            try {
                ...
                $this->log(rPDO::LOG_LEVEL_ERROR, "An exception occurred calling {$className}::{$method}() - " . $e->getMessage());
            }
        } else {
            $this->log(rPDO::LOG_LEVEL_ERROR, "{$class}::{$method}() is not a valid static method.");
        }
        return $return;
    }
     ...
     * rPDOObject::save()} is called the first time and is reflected by the
     * {@link Om\rPDOObject::$_new} property.
     *
     * @template T of Om\rPDOObject
     ...
     * (for multiple primary key objects) or an {@link rPDOCriteria} object. If
     ...
     *
     * @uses rPDOObject::load()
     * @template T of Om\rPDOObject
     * @param class-string<T> $className Name of the class to get an instance of.
     * @param mixed $criteria Primary key of the record or a rPDOCriteria object.
    ...
    */
    ...
    /**
     * Retrieves a collection of rPDOObjects by the specified rPDOCriteria.
     *
     * @uses rPDOObject::loadCollection()
     * @template T of Om\rPDOObject
     * @param class-string<T> $className Name of the class to search for instances of.
     * @param object|array|string $criteria An rPDOCriteria object or an array
     * search expression.
     * @param mixed $cacheFlag If an integer value is provided, this specifies
    ...
    */
    ...
    /**
     * Retrieves an iterable representation of a collection of rPDOObjects.
     *
     * @param string $className Name of the class to search for instances of.
     * @param mixed $criteria An rPDOCriteria object or representation.
     * @param bool $cacheFlag If an integer value is provided, this specifies
     * the time to live in the result set cache; if cacheFlag === false, caching
     * is ignored for the collection and if cacheFlag === true, the objects will
     * live in cache until flushed by another process.
     * @return rPDOIterator An iterable representation of a collection.
     */
    public function getIterator($className, $criteria= null, $cacheFlag= true) {
        return new rPDOIterator($this, array('class' => $className, 'criteria' => $criteria, 'cacheFlag' => $cacheFlag));
    }

    /**
     * Update field values across a collection of rPDOObjects.
     *
    ...
    public function updateCollection($className, array $set, $criteria= null) {
        $affected = false;
        if ($this->getConnection(array(rPDO::OPT_CONN_MUTABLE => true))) {
            ...
            if ($query && !empty($set)) {
                ...
                if ($query->prepare()) {
                    ...
                    if ($affected === false) {
                        $this->log(rPDO::LOG_LEVEL_ERROR, "Error updating {$className} instances using query " . $query->toSQL(), '', __METHOD__, __FILE__, __LINE__);
                    } else {
                        if ($this->getOption(rPDO::OPT_CACHE_DB)) {
                            ...
                            foreach ($relatedClasses as $relatedClass) {
                                $this->cacheManager->delete($relatedClass, array(
                                    rPDO::OPT_CACHE_KEY => $this->getOption('cache_db_key', null, 'db'),
                                    rPDO::OPT_CACHE_HANDLER => $this->getOption(rPDO::OPT_CACHE_DB_HANDLER, null, $this->getOption(rPDO::OPT_CACHE_HANDLER, null, 'rPDO\\Cache\\rPDOFileCache')),
                                    rPDO::OPT_CACHE_FORMAT => (integer) $this->getOption('cache_db_format', null, $this->getOption(rPDO::OPT_CACHE_FORMAT, null, Cache\rPDOCacheManager::CACHE_PHP)),
                                    rPDO::OPT_CACHE_PREFIX => $this->getOption('cache_db_prefix', null, Cache\rPDOCacheManager::CACHE_DIR),
                                    rPDO::OPT_CACHE_MULTIPLE_OBJECT_DELETE => true
                                ));
                            }
                        }
                        $callback = $this->getOption(rPDO::OPT_CALLBACK_ON_SAVE);
            ...
            $this->log(rPDO::LOG_LEVEL_ERROR, "Could not get connection for writing data", '', __METHOD__, __FILE__, __LINE__);
        }
        return $affected;
    }
     ...
     * @param mixed $criteria Valid rPDO criteria for selecting an instance.
    ...
    public function removeObject($className, $criteria) {
        ...
        if ($this->getConnection(array(rPDO::OPT_CONN_MUTABLE => true))) {
            if ($this->getCount($className, $criteria) === 1) {
                if ($query= $this->newQuery($className)) {
                    ...
                    if ($query->prepare()) {
                        if ($this->exec($query->toSQL()) !== 1) {
                            $this->log(rPDO::LOG_LEVEL_ERROR, "rPDO->removeObject - Error deleting {$className} instance using query " . $query->toSQL());
                        } else {
                            $removed= true;
                            if ($this->getOption(rPDO::OPT_CACHE_DB)) {
                                $this->cacheManager->delete(Cache\rPDOCacheManager::CACHE_DIR . $query->getAlias(), array(rPDO::OPT_CACHE_MULTIPLE_OBJECT_DELETE => true));
                            }
                            $callback = $this->getOption(rPDO::OPT_CALLBACK_ON_REMOVE);
                            if ($callback && is_callable($callback)) {
                ...
                $this->log(rPDO::LOG_LEVEL_WARN, "rPDO->removeObject - {$className} instance to remove not found!");
                if ($this->getDebug() === true) $this->log(rPDO::LOG_LEVEL_DEBUG, "rPDO->removeObject - {$className} instance to remove not found using criteria " . print_r($criteria, true));
            }
        } else {
            $this->log(rPDO::LOG_LEVEL_ERROR, "Could not get connection for writing data", '', __METHOD__, __FILE__, __LINE__);
        }
     ...
     * @param mixed $criteria Valid rPDO criteria for selecting a collection.
        ...
        if ($this->getConnection(array(rPDO::OPT_CONN_MUTABLE => true))) {
            if ($query= $this->newQuery($className)) {
                ...
                if ($query->prepare()) {
                    ...
                    if ($removed === false) {
                        $this->log(rPDO::LOG_LEVEL_ERROR, "rPDO->removeCollection - Error deleting {$className} instances using query " . $query->toSQL());
                    } else {
                        if ($this->getOption(rPDO::OPT_CACHE_DB)) {
                            $this->cacheManager->delete(Cache\rPDOCacheManager::CACHE_DIR . $query->getAlias(), array(rPDO::OPT_CACHE_MULTIPLE_OBJECT_DELETE => true));
                        }
                    ...
                    $this->log(rPDO::LOG_LEVEL_ERROR, "rPDO->removeCollection - Error preparing statement to delete {$className} instances using query: {$query->toSQL()}");
                }
            }
        } else {
            $this->log(rPDO::LOG_LEVEL_ERROR, "Could not get connection for writing data", '', __METHOD__, __FILE__, __LINE__);
    ...
    /**
     * Retrieves a count of rPDOObjects by the specified rPDOCriteria.
     *
     * @param string $className Class of rPDOObject to count instances of.
     * @param mixed $criteria Any valid rPDOCriteria object or expression.
    ...
    public function getCount($className, $criteria = null) {
        ...
        if ($query = $this->newQuery($className, $criteria)) {
            ...
            if (!empty($query->query['groupby']) || !empty($query->query['having'])) {
                $query->select($expr);
                if ($query->prepare()) {
                    $countQuery = new rPDOCriteria($this, "SELECT COUNT(*) FROM ({$query->toSQL(false)}) cq", $query->bindings, $query->cacheFlag);
    ...
    /**
     * Retrieves an rPDOObject instance with specified related objects.
     *
     * @uses rPDO::getCollectionGraph()
     * @template T of Om\rPDOObject
     ...
     * @param mixed $criteria A valid rPDOCriteria instance or expression.
    ...
    public function getObjectGraph($className, $graph, $criteria= null, $cacheFlag= true) {
        ...
        if ($collection= $this->getCollectionGraph($className, $graph, $criteria, $cacheFlag)) {
            if (!count($collection) === 1) {
                $this->log(rPDO::LOG_LEVEL_WARN, 'getObjectGraph criteria returned more than one instance.');
            }
    ...
    /**
     * Retrieves a collection of rPDOObject instances with related objects.
     *
     * @uses rPDOQuery::bindGraph()
     * @template T of Om\rPDOObject
     ...
     * or {"relationAlias":{"subRelationAlias":{}}}.  Note that the empty arrays
     * are necessary in order for the relation to be recognized.
     * @param mixed $criteria A valid rPDOCriteria instance or condition string.
    ...
    public function getValue($stmt, $column= null) {
        ...
        if (is_object($stmt) && $stmt instanceof \PDOStatement) {
            ...
            if ($stmt->execute()) {
                ...
                $this->queryTime += microtime(true) - $tstart;
                $this->executedQueries++;
                $this->log(rPDO::LOG_LEVEL_ERROR, "Error " . $stmt->errorCode() . " executing statement: \n" . print_r($stmt->errorInfo(), true), '', __METHOD__, __FILE__, __LINE__);
            }
        } else {
            $this->log(rPDO::LOG_LEVEL_ERROR, "No valid PDOStatement provided to getValue", '', __METHOD__, __FILE__, __LINE__);
        }
        return $value;
    }

    /**
     * Convert any valid criteria into an rPDOQuery instance.
     *
     * @todo Get criteria pre-defined in an {@link rPDOObject} class metadata
     * definition by name.
     *
     ...
     * @return Om\rPDOCriteria A criteria object or null if not found.
     */
     ...
     *
     * @param mixed $criteria An rPDOCriteria instance or any valid criteria variable.
    ...
    public function getCriteriaType($criteria) {
        ...
        if ($type === 'object') {
            ...
            if (!$criteria instanceof Om\rPDOCriteria) {
                $this->log(rPDO::LOG_LEVEL_WARN, "Invalid criteria object of class {$type} encountered.", '', __METHOD__, __FILE__, __LINE__);
                $type = null;
            } elseif ($criteria instanceof Om\rPDOQuery) {
                $type = 'rPDOQuery';
            } else {
                $type = 'rPDOCriteria';
            }
     ...
     *
     * @param string $className A valid rPDOObject derivative table class.
     * @param Om\rPDOQuery $criteria A valid rPDOQuery instance.
     * @return Om\rPDOQuery The rPDOQuery instance with derivative criteria added.
     */
    public function addDerivativeCriteria($className, $criteria) {
        if ($criteria instanceof Om\rPDOQuery && ($className = $this->loadClass($className)) && !isset($this->map[$className]['table'])) {
            if (isset($this->map[$className]['fields']['class_key']) && !empty($this->map[$className]['fields']['class_key'])) {
                ...
                if ($this->getDebug() === true) {
                    $this->log(rPDO::LOG_LEVEL_DEBUG, "#1: Automatically adding class_key criteria for derivative query of class {$className}");
                }
            } else {
                foreach ($this->getAncestry($className, false) as $ancestor) {
                    if (isset($this->map[$ancestor]['table']) && isset($this->map[$ancestor]['fields']['class_key'])) {
                        ...
                        if ($this->getDebug() === true) {
                            $this->log(rPDO::LOG_LEVEL_DEBUG, "#2: Automatically adding class_key criteria for derivative query of class {$className} from base table class {$ancestor}");
                        }
                        break;
                    }
                }
    ...
    public function getService($name, $class= '', $path= '', $params= array ()) {
        ...
        if ($this->services->has($name)) {
            ...
            if ($this->getDebug() === true) {
                $this->log(rPDO::LOG_LEVEL_DEBUG, "Problem getting service {$name}, instance of class {$class}, from path {$path}, with params " . print_r($params, true));
            } else {
                $this->log(rPDO::LOG_LEVEL_ERROR, "Problem getting service {$name}, instance of class {$class}, from path {$path}");
            }
        }
        return $service;
    }
    ...
    public function getTableName($className, $includeDb= false) {
        ...
        if ($className= $this->loadClass($className)) {
            if (isset ($this->map[$className]['table'])) {
                ...
                if (isset($this->map[$className]['package']) && isset($this->packages[$this->map[$className]['package']]['prefix'])) {
                    ...
                    $table= $this->getOption(rPDO::OPT_TABLE_PREFIX, null, '') . $table;
                }
            }
        ...
        if ($table) {
            ...
            if ($this->getDebug() === true) $this->log(rPDO::LOG_LEVEL_DEBUG, 'Returning table name: ' . $table . ' for class: ' . $className);
        } else {
            $this->log(rPDO::LOG_LEVEL_ERROR, 'Could not get table name for class: ' . $className);
        }
        return $table;
    }
    ...
    public function getTableClass($className) {
        ...
        if ($tableClass) {
            if ($this->getDebug() === true) {
                $this->log(rPDO::LOG_LEVEL_DEBUG, 'Returning table class: ' . $tableClass . ' for class: ' . $className);
            }
        } else {
            $this->log(rPDO::LOG_LEVEL_ERROR, 'Could not get table class for class: ' . $className);
        }
        return $tableClass;
    }
    ...
    /**
     * Indicates the inheritance model for the rPDOObject class specified.
     *
    ...
    public function getInherit($className) {
        ...
        if (!empty($inherit)) {
            if ($this->getDebug() === true) {
                $this->log(rPDO::LOG_LEVEL_DEBUG, 'Returning inherit: ' . $inherit . ' for class: ' . $className);
            }
        } else {
            $inherit= 'none';
        }
    ...
    public function getValidationRules($className) {
        ...
        if ($className= $this->loadClass($className)) {
            ...
            if ($this->getDebug() === true) {
                $this->log(rPDO::LOG_LEVEL_DEBUG, "Returning validation rules: " . print_r($rules, true));
            }
        }
        return $rules;
    }
    ...
    public function getIndexMeta($className) {
        ...
        if ($className= $this->loadClass($className)) {
            if ($ancestry= $this->getAncestry($className)) {
                ...
                if ($this->getDebug() === true) {
                    $this->log(rPDO::LOG_LEVEL_DEBUG, "Returning indices: " . print_r($indices, true));
                }
            }
        }
        return $indices;
    ...
    public function getPK($className) {
        $pk= null;
        if (strcasecmp($className, 'rPDOObject') !== 0) {
            if ($actualClassName= $this->loadClass($className)) {
                ...
                $this->log(rPDO::LOG_LEVEL_ERROR, "Could not load class {$className}");
            }
        }
        return $pk;
    }
    ...
    /**
     * Get a complete relation graph for an rPDOObject class.
     *
     * @param string $className A fully-qualified rPDOObject class name.
     ...
     * @return array An rPDOObject relation graph, or an empty array if no graph can be constructed.
     */
    ...
    public function getAncestry($className, $includeSelf= true) {
        ...
        if ($actualClassName= $this->loadClass($className)) {
            ...
            if ($this->getDebug() === true) {
                $this->log(rPDO::LOG_LEVEL_DEBUG, "Returning ancestry for {$className}: " . print_r($ancestry, 1));
            }
        }
        return $ancestry;
    }
     ...
     *
     * @uses rPDOObject::getSelectColumns()
    ...
    function getFKDefinition($parentClass, $alias) {
        ...
        if ($def === null) {
            $this->log(rPDO::LOG_LEVEL_ERROR, 'No foreign key definition for parentClass: ' . $parentClass . ' using relation alias: ' . $alias);
        }
        return $def;
    }
    ...
    /**
     * OK!!!
     * Получает класс manager для этого соединения rPDO.
     *
     ...
     * часто.
     *
     * @return Om\rPDOManager|null Экземпляр rPDOManager для подключения rPDO или null
     * если класс менеджера не может быть создан.
     */
    public function getManager() {
        if ($this->manager === null || !$this->manager instanceof Om\rPDOManager) {
            $managerClass = '\\rPDO\\Om\\' . $this->config['dbtype'] . '\\rPDOManager';
            $this->manager= new $managerClass($this);
            if (!$this->manager) {
                $this->log(rPDO::LOG_LEVEL_ERROR, "Не удалось загрузить класс rPDOManager.");
            }
        }
        return $this->manager;
    }
     ...
     *
     * @return Om\rPDODriver|null Экземпляр rPDODriver для подключения rPDO или null
     * если класс драйвера не может быть создан.
     */
    public function getDriver() {
        if ($this->driver === null || !$this->driver instanceof Om\rPDODriver) {
            $driverClass = '\\rPDO\\Om\\' . $this->config['dbtype'] . '\\rPDODriver';
            $this->driver= new $driverClass($this);
            if (!$this->driver) {
                $this->log(rPDO::LOG_LEVEL_ERROR, "Не удалось загрузить класс драйвера rPDO для {$this->config['dbtype']} PDO-драйвера.");
            }
        }
        return $this->driver;
    }
    ...
    /**
     * OK!!!
     * Получает экземпляр rPDOCacheManager.
     *
     * Этот класс отвечает за обработку всех типов операций кэширования для ядра rPDO.
     *
     * @param string $class Необязательное имя производного класса rPDOCacheManager.
     ...
     * - ignorePkg = Если значение равно false и вы не указываете путь, вы можете просмотреть пользовательский rPDOCacheManager
    ...
    /**
     * OK!!!
     * Возвращает состояние отладки для экземпляра rPDO.
     *
    ...
    /**
     * OK!!!
     * Устанавливает состояние отладки для экземпляра rPDO.
     *
    ...
    /**
     * OK!!!
     * Устанавливает состояние уровня ведения журнала для экземпляра rPDO.
     *
     * @param integer $level Уровень ведения журнала, на который нужно переключиться.
     * @return integer Предыдущий уровень журнала.
     */
    public function setLogLevel($level= rPDO::LOG_LEVEL_FATAL) {
    ...
    /**
     * OK!!!
     * Устанавливает целевое значение журнала для вызовов rPDO::_log().
     *
    ...
    protected function _log($level, $msg, $target= '', $def= '', $file= '', $line= '') {
        if ($level !== rPDO::LOG_LEVEL_FATAL && $level > $this->logLevel && $this->_debug !== true) {
            return;
        }
        if (empty ($target)) {
        ...
        if ($this->_debug === true || $level <= $this->logLevel) {
            ...
            if ($target=='FILE' && $this->getCacheManager()) {
                ...
                $filepath = isset($targetOptions['filepath']) ? $targetOptions['filepath'] : $this->getCachePath() . Cache\rPDOCacheManager::LOG_DIR;
     ...
     *
     * Эта функция возвращает только поля, возвращаемые с помощью rPDOObject::toArray()
     * в экземплярах rPDOObject и просто имя класса для других объектов, чтобы
    ...
    public function getDebugBacktrace() {
        ...
        foreach (debug_backtrace() as $levelKey => $levelElement) {
            foreach ($levelElement as $traceKey => $traceElement) {
                // if ($traceKey == 'object' && $traceElement instanceof Om\rPDOObject) {
    ...
    protected function _getLogLevel($level) {
        switch ($level) {
            case rPDO::LOG_LEVEL_DEBUG :
                $levelText= 'DEBUG';
                break;
            case rPDO::LOG_LEVEL_INFO :
                $levelText= 'INFO';
                break;
            case rPDO::LOG_LEVEL_WARN :
                $levelText= 'WARN';
                break;
            case rPDO::LOG_LEVEL_ERROR :
                $levelText= 'ERROR';
                break;
            default :
                $levelText= 'FATAL';
     ...
     * queries with rPDO, it is highly recommend to escape any database or column names in any native SQL strings used.
     *
     ...
     *
     * @param string|Om\rPDOCriteria $signature A unique string or rPDOCriteria object
    ...
    public function fromCache($signature, $class= '', $options= array()) {
        $result= null;
        if ($this->getOption(rPDO::OPT_CACHE_DB, $options)) {
            if ($signature && $this->getCacheManager()) {
                ...
                if (is_object($signature)) {
                    if ($signature instanceof Om\rPDOCriteria) {
                        if ($signature instanceof Om\rPDOQuery) {
                            $signature->construct();
                ...
                if (is_string($sig) && !empty($sig)) {
                    $result= $this->cacheManager->get($sig, array(
                        rPDO::OPT_CACHE_KEY => $this->getOption('cache_db_key', $options, 'db'),
                        rPDO::OPT_CACHE_HANDLER => $this->getOption(rPDO::OPT_CACHE_DB_HANDLER, $options, $this->getOption(rPDO::OPT_CACHE_HANDLER, $options, 'rPDO\\Cache\\rPDOFileCache')),
                        rPDO::OPT_CACHE_FORMAT => (integer) $this->getOption('cache_db_format', null, $this->getOption(rPDO::OPT_CACHE_FORMAT, null, Cache\rPDOCacheManager::CACHE_PHP)),
                        'cache_prefix' => $this->getOption('cache_db_prefix', $options, Cache\rPDOCacheManager::CACHE_DIR),
                    ));
                    if ($this->getDebug() === true) {
                        if (!$result) {
                            $this->log(rPDO::LOG_LEVEL_DEBUG, 'No cache item found for class ' . $sigClass . ' with signature ' . Cache\rPDOCacheManager::CACHE_DIR . $sig);
                        } else {
                            $this->log(rPDO::LOG_LEVEL_DEBUG, 'Loaded cache item for class ' . $sigClass . ' with signature ' . Cache\rPDOCacheManager::CACHE_DIR . $sig);
                        }
                    }
                }
            }
    ...
    public function toCache($signature, $object, $lifetime= 0, $options = array()) {
        ...
        if ($this->getCacheManager()) {
            if ($this->getOption(rPDO::OPT_CACHE_DB, $options)) {
                ...
                elseif (!$lifetime && $this->getOption(rPDO::OPT_CACHE_DB_EXPIRES, $options, 0)) {
                    $lifetime= intval($this->getOption(rPDO::OPT_CACHE_DB_EXPIRES, $options, 0));
                }
                $sigKey= array();
                $sigClass= '';
                $sigGraph= $this->getOption(rPDO::OPT_CACHE_DB_SIG_GRAPH, $options, array());
                if (is_object($signature)) {
                    if ($signature instanceof Om\rPDOCriteria) {
                        if ($signature instanceof Om\rPDOQuery) {
                            $signature->construct();
                ...
                if (empty($sigClass)) {
                    // if ($object instanceof Om\rPDOObject) {
                    //     $sigClass= $object->_class;
                    // } else {
                        $sigClass= $this->getOption(rPDO::OPT_CACHE_DB_SIG_CLASS, $options, '__sqlResult');
                    // }
                }
                if (empty($sigKey) && is_string($signature)) $sigKey= $signature;
                // if (empty($sigKey) && $object instanceof Om\rPDOObject) $sigKey= $object->getPrimaryKey();
                if ($sigClass && $sigKey) {
                    $sigHash= md5($this->toJSON(is_array($sigKey) ? $sigKey : array($sigKey)));
                    $sig= implode('/', array ($sigClass, $sigHash));
                    if (is_string($sig)) {
                        if ($this->getOption('modified', $options, false)) {
                            // if (empty($sigGraph) && $object instanceof Om\rPDOObject) {
                            ...
                            if (!empty($sigGraph)) {
                                foreach ($sigGraph as $gAlias => $gMeta) {
                                    ...
                                    $removed= $this->cacheManager->delete($gClass, array_merge($options, array(
                                        rPDO::OPT_CACHE_KEY => $this->getOption('cache_db_key', $options, 'db'),
                                        rPDO::OPT_CACHE_HANDLER => $this->getOption(rPDO::OPT_CACHE_DB_HANDLER, $options, $this->getOption(rPDO::OPT_CACHE_HANDLER, $options, 'rPDO\\Cache\\rPDOFileCache')),
                                        rPDO::OPT_CACHE_FORMAT => (integer) $this->getOption('cache_db_format', $options, $this->getOption(rPDO::OPT_CACHE_FORMAT, $options, Cache\rPDOCacheManager::CACHE_PHP)),
                                        rPDO::OPT_CACHE_EXPIRES => (integer) $this->getOption(rPDO::OPT_CACHE_DB_EXPIRES, null, $this->getOption(rPDO::OPT_CACHE_EXPIRES, null, 0)),
                                        rPDO::OPT_CACHE_PREFIX => $this->getOption('cache_db_prefix', $options, Cache\rPDOCacheManager::CACHE_DIR),
                                        rPDO::OPT_CACHE_MULTIPLE_OBJECT_DELETE => true
                                    )));
                                    if ($this->getDebug() === true) {
                                        $this->log(rPDO::LOG_LEVEL_DEBUG, "Removing all cache objects of class {$gClass}: " . ($removed ? 'successful' : 'failed'));
                                    }
                                }
                            }
                        }
                        ...
                        // if ($result && $object instanceof Om\rPDOObject) {
                            // if ($this->getDebug() === true) {
                            //     $this->log(rPDO::LOG_LEVEL_DEBUG, "rPDO->toCache() successfully cached object with signature " . Cache\rPDOCacheManager::CACHE_DIR . $sig);
                            // }
                        // }
                        if (!$result) {
                            $this->log(rPDO::LOG_LEVEL_WARN, "rPDO->toCache() could not cache object with signature " . Cache\rPDOCacheManager::CACHE_DIR . $sig);
                        }
                    }
                } else {
                    $this->log(rPDO::LOG_LEVEL_ERROR, "Object sent toCache() has an invalid signature.");
                }
            }
        } else {
            $this->log(rPDO::LOG_LEVEL_ERROR, "Attempt to send a non-object to toCache().");
        }
        return $result;
    }
     ...
     *
     * @throws rPDOException Если json_encode недоступен.
     ...
     *
     * @throws rPDOException Если json_decode недоступен.
    ...
    public function beginTransaction() {
        if (!$this->connect(null, array(rPDO::OPT_CONN_MUTABLE => true))) {
            return false;
        }
    ...
    /**
     * Создает новый rPDOQuery для указанного класса rPDOObject.
     *
     * @param string $class Класс, для которого нужно создать rPDOQuery.
     * @param mixed $criteria Любое допустимое выражение критерия rPDO.
    ...
    public function parseBindings($sql, $bindings) {
        if (!empty($sql) && !empty($bindings)) {
            ...
            if ($this->getDebug() === true) {
                $this->log(rPDO::LOG_LEVEL_DEBUG, "{$sql}\n" . print_r($bound, true));
            }
            if (!empty($bound)) {
