...
namespace rPDO\Reflect;

use ReflectionClass;
use ReflectionFunctionAbstract;
use ReflectionProperty;
use rPDO\Om\rPDOGenerator;
use rPDO\rPDOException;

class rPDOReflectionClass extends ReflectionClass
{
        ...
        if ($element instanceof ReflectionClass || $element instanceof ReflectionFunctionAbstract) {
            if (is_readable($element->getFileName())) {
                try {
                    ...
                    throw new rPDOException("Error getting source from Reflection element: {$e->getMessage()}");
                }
            }
            ...
            if (array_key_exists($element->getName(),
                ...
                $source .= ' = ' . rPDOGenerator::varExport($this->defaultProperties[$element->getName()], 1);
            }
            $source .= ';';
        }
        return $source;
