...
use rPDO\Reflect\rPDOReflectionClass;
use rPDO\rPDO;

/**
 * Сервис для обратного и форвардного проектирования моделей предметной области rPDO.
 *
 * Эта служба использует экземпляр rPDOManager для генерации заглушки класса и
...
abstract class rPDOGenerator {
    ...
    /**
     * @var rPDOManager $manager A reference to the xPDOManager using this
     * generator.
     */
    public $manager= null;
    /**
    ...
    public function parseSchema($schemaFile, $outputDir= '', $options = array()) {
        ...
        if (!is_file($schemaFile)) {
            $this->manager->xpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not find specified XML schema file {$schemaFile}");
            return false;
        }
        ...
        if (isset($this->schema)) {
            ...
            if (isset($this->schema->object)) {
                ...
                $this->manager->xpdo->log(rPDO::LOG_LEVEL_ERROR, "Schema {$schemaFile} contains no valid object elements.");
            }
        } else {
            $this->manager->xpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not read schema from {$schemaFile}.");
        }
            ...
            if (!in_array($classFullName, self::$updated)) {
                if ($newClass || $regenerate === 2) {
                    ...
                    if (!$this->_constructClass($fileName, $classDef, $this->getClassTemplate())) {
                        $this->manager->xpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not construct domain class {$classFullName} to file {$fileName}", '', __METHOD__, __FILE__, __LINE__);
                    }
                } elseif (!$newClass && $update === 2) {
                    $this->_loadExistingClass($classFullName, $classDef);
                    self::$updated[] = $classFullName;
                    if (!$this->_constructClass($fileName, $classDef, $this->getClassTemplate())) {
                        $this->manager->xpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not reconstruct domain class {$classFullName} to file {$fileName}", '', __METHOD__, __FILE__, __LINE__);
                    }
                } else {
                    $this->manager->xpdo->log(rPDO::LOG_LEVEL_INFO, "Skipping {$fileName}: Use update or regenerate options to overwrite or update your domain classes.");
                }
            } else {
                $this->manager->xpdo->log(rPDO::LOG_LEVEL_WARN, "Domain class {$classFullName} was already constructed to file {$fileName} in this session", '', __METHOD__, __FILE__, __LINE__);
            }
            ...
            if (!in_array($platformClass, self::$updated)) {
                ...
                if ($newPlatformClass || $regenerate > 0) {
                    ...
                    if (!$this->_constructClass($fileName, $classDef, $this->getClassPlatformTemplate())) {
                        $this->manager->xpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not construct platform class {$platformClass} to file {$fileName}", '', __METHOD__, __FILE__, __LINE__);
                    }
                } elseif (!$newClass && $update > 0) {
                    $this->_loadExistingClass($platformClass, $classDef);
                    self::$updated[] = $platformClass;
                    if (!$this->_constructClass($fileName, $classDef, $this->getClassPlatformTemplate())) {
                        $this->manager->xpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not reconstruct platform class {$platformClass} to file {$fileName}", '', __METHOD__, __FILE__, __LINE__);
                    }
                } else {
                    $this->manager->xpdo->log(rPDO::LOG_LEVEL_INFO, "Skipping {$fileName}: Use update or regenerate options to overwrite or update your platform classes.");
                }
            } else {
                $this->manager->xpdo->log(rPDO::LOG_LEVEL_WARN, "Platform class {$platformClass} was already constructed to file {$fileName} in this session", '', __METHOD__, __FILE__, __LINE__);
            }
        }
    }
        ...
        if (!is_dir($path)) {
            if ($this->manager->xpdo->getCacheManager()) {
                if (!$this->manager->xpdo->cacheManager->writeTree($path)) {
                    $this->manager->xpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not create model directory at {$path}");
                    return false;
                }
            }
        }
...
EOD;
    ...
    protected function _loadExistingClass($class, &$meta) {
        try {
            $reflector = new rPDOReflectionClass($class);
            ...
            $this->manager->xpdo->log(rPDO::LOG_LEVEL_ERROR, $e->getMessage(), '', __METHOD__, __FILE__, __LINE__);
        }
    }
    ...
    protected function _constructPhpDocProperties($meta) {
        ...
        if ($this->manager->xpdo->getOption(rPDO::OPT_HYDRATE_FIELDS)) {
        ...
        if ($this->manager->xpdo->getOption(rPDO::OPT_HYDRATE_RELATED_OBJECTS)) {
