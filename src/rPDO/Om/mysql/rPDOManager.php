...
class rPDOManager extends \rPDO\Om\rPDOManager {
    public function createSourceContainer($dsnArray = null, $username= null, $password= null, $containerOptions= array ()) {
        $created= false;
        if ($this->vpdo->getConnection(array(rPDO::OPT_CONN_MUTABLE => true))) {
            if ($dsnArray === null) $dsnArray = rPDO::parseDSN($this->vpdo->getOption('dsn'));
            ...
            if (is_array($dsnArray) && is_string($username) && is_string($password)) {
                ...
                try {
                    ...
                    if ($result !== false) {
                        ...
                        $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not create source container:\n{$sql}\nresult = " . var_export($result, true));
                    }
                } catch (\PDOException $pe) {
                    $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not connect to database server: " . $pe->getMessage());
                } catch (\Exception $e) {
                    $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not create source container: " . $e->getMessage());
                }
            }
        } else {
            $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not get writable connection", '', __METHOD__, __FILE__, __LINE__);
        }
        return $created;
    }
            ...
            if (is_array($dsnArray) && is_string($username) && is_string($password)) {
                ...
                try {
                    ...
                    if ($result !== false) {
                        ...
                        $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not remove source container:\n{$sql}\nresult = " . var_export($result, true));
                    }
                } catch (\PDOException $pe) {
                    $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not connect to database server: " . $pe->getMessage());
                } catch (\Exception $e) {
                    $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not remove source container: " . $e->getMessage());
                }
            }
        } else {
            $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not get writable connection", '', __METHOD__, __FILE__, __LINE__);
        ...
        if ($this->vpdo->getConnection(array(rPDO::OPT_CONN_MUTABLE => true))) {
            ...
            if ($instance) {
                ...
                if ($removed === false && $this->vpdo->errorCode() !== '' && $this->vpdo->errorCode() !== \PDO::ERR_NONE) {
                    $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, 'Could not drop table ' . $className . "\nSQL: {$sql}\nERROR: " . print_r($this->vpdo->pdo->errorInfo(), true));
                } else {
                    $removed= true;
                    $this->vpdo->log(rPDO::LOG_LEVEL_INFO, 'Dropped table' . $className . "\nSQL: {$sql}\n");
                }
            }
        } else {
            $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not get writable connection", '', __METHOD__, __FILE__, __LINE__);
    ...
    public function createObjectContainer($className) {
        ...
        if ($this->vpdo->getConnection(array(rPDO::OPT_CONN_MUTABLE => true))) {
            ...
            if ($instance) {
                ...
                $tableType= $this->vpdo->getOption(rPDO::OPT_OVERRIDE_TABLE_TYPE, null, $tableType);
                ...
                if ($created === false && $this->vpdo->errorCode() !== '' && $this->vpdo->errorCode() !== \PDO::ERR_NONE) {
                    $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, 'Could not create table ' . $tableName . "\nSQL: {$sql}\nERROR: " . print_r($this->vpdo->errorInfo(), true));
                } else {
                    $created= true;
                    $this->vpdo->log(rPDO::LOG_LEVEL_INFO, 'Created table ' . $tableName . "\nSQL: {$sql}\n");
                }
            }
        } else {
            $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not get writable connection", '', __METHOD__, __FILE__, __LINE__);
                ...
                if (is_array($meta) && array_key_exists($name, $meta)) {
                    ...
                    if (!empty($colDef)) {
                        ...
                        if ($this->vpdo->exec($sql) !== false) {
                            ...
                            $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Error adding field {$class}->{$name}: " . print_r($this->vpdo->errorInfo(), true), '', __METHOD__, __FILE__, __LINE__);
                        }
                    } else {
                        $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Error adding field {$class}->{$name}: Could not get column definition");
                    }
                } else {
                    $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Error adding field {$class}->{$name}: No metadata defined");
                }
            }
        } else {
            $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not get writable connection", '', __METHOD__, __FILE__, __LINE__);
                ...
                if (is_array($meta) && array_key_exists($name, $meta)) {
                    ...
                    if (!empty($idxDef)) {
                        ...
                        if ($this->vpdo->exec($sql) !== false) {
                            ...
                            $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Error adding index {$name} to {$class}: " . print_r($this->vpdo->errorInfo(), true), '', __METHOD__, __FILE__, __LINE__);
                        }
                    } else {
                        $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Error adding index {$name} to {$class}: Could not get index definition");
                    }
                } else {
                    $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Error adding index {$name} to {$class}: No metadata defined");
                }
            }
        } else {
            $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not get writable connection", '', __METHOD__, __FILE__, __LINE__);
                ...
                if (is_array($meta) && array_key_exists($name, $meta)) {
                    ...
                    if (!empty($colDef)) {
                        ...
                        if ($this->vpdo->exec($sql) !== false) {
                            ...
                            $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Error altering field {$class}->{$name}: " . print_r($this->vpdo->errorInfo(), true), '', __METHOD__, __FILE__, __LINE__);
                        }
                    } else {
                        $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Error altering field {$class}->{$name}: Could not get column definition");
                    }
                } else {
                    $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Error altering field {$class}->{$name}: No metadata defined");
                }
            }
        } else {
            $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not get writable connection", '', __METHOD__, __FILE__, __LINE__);
        ...
        if ($this->vpdo->getConnection(array(rPDO::OPT_CONN_MUTABLE => true))) {
            ...
            if ($className) {
                ...
                if ($this->vpdo->exec($sql) !== false) {
                    ...
                    $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Error removing field {$class}->{$name}: " . print_r($this->vpdo->errorInfo(), true), '', __METHOD__, __FILE__, __LINE__);
                }
            }
        } else {
            $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not get writable connection", '', __METHOD__, __FILE__, __LINE__);
        ...
        if ($this->vpdo->getConnection(array(rPDO::OPT_CONN_MUTABLE => true))) {
            ...
            if ($className) {
                ...
                if ($this->vpdo->exec($sql) !== false) {
                    ...
                    $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Error removing index {$name} from {$class}: " . print_r($this->vpdo->errorInfo(), true), '', __METHOD__, __FILE__, __LINE__);
                }
            }
        } else {
            $this->vpdo->log(rPDO::LOG_LEVEL_ERROR, "Could not get writable connection", '', __METHOD__, __FILE__, __LINE__);
