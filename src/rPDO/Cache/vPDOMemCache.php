...
/**
 * Предоставляет реализацию rPDOCache на базе memcache.
 *
...
class rPDOMemCache extends rPDOCache {
    ...
    public function add($key, $var, $expire= 0, $options= array()) {
        $added= $this->memcache->add(
            ...
            $var,
            $this->getOption($this->key . rPDO::OPT_CACHE_COMPRESS, $options, $this->getOption(rPDO::OPT_CACHE_COMPRESS, $options, false)),
            $expire
        );
        return $added;
    }
