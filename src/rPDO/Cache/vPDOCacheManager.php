...
/**
 * Реализация диспетчера кэша по умолчанию для rPDO.
 *
 * @package rPDO\Cache
 */
class rPDOCacheManager {
    const CACHE_PHP = 0;
    const CACHE_JSON = 1;
    ...
    /**
     * Получите экземпляр поставщика, который реализует интерфейс rPDOCache.
     *
     * @param string $key
     * @param array $options
     *
     * @return rPDOCache|null
     */
    public function getCacheProvider($key = '', $options = array()) {
        $objCache = null;
        if (empty($key)) {
            $key = $this->getOption(rPDO::OPT_CACHE_KEY, $options, 'default');
        }
     ...
     * или реализации rPDOCacheManager.
     ...
     *
     * @access public
     * @return string Абсолютный путь к каталогу кэша rPDO.
     */
    ...
    public function writeFile($filename, $content, $mode= 'wb', $options= array()) {
        ...
        if ($file) {
            if ($append === true) {
                ...
                $locked = false;
                $attempt = 1;
                $attempts = (integer) $this->getOption(rPDO::OPT_CACHE_ATTEMPTS, $options, 1);
                $attemptDelay = (integer) $this->getOption(rPDO::OPT_CACHE_ATTEMPT_DELAY, $options, 1000);
     ...
     *
     * rPDO::OPT_USE_FLOCK должно быть установлено значение false (или 0), иначе rPDO будет считать flock надежным.
     *
    ...
    protected function lockFileName($file, array $options = array()) {
        $lockDir = $this->getOption('lock_dir', $options, $this->getCachePath() . 'locks' . DIRECTORY_SEPARATOR);
        return $lockDir . preg_replace('/\W/', '_', $file) . $this->getOption(rPDO::OPT_LOCKFILE_EXTENSION, $options, '.lock');
    }
    ...
    /**
     * Сгенерируйте исполняемое на PHP представление rPDOObject.
     *
     * @todo Полная функциональность, связанная с $generateRelated.
     * @todo Добавьте поддержку stdObject.
     *
     * @access public
     * @param \rPDO\Om\rPDOObject $obj Объект rPDOObject для генерации файла кэша для
     * @param string $objName Имя rPDOObject
     ...
     * @param string $objRef Ссылка на экземпляр rPDO в строке формат.
     * @param int $format Формат для кэширования. По умолчанию используется
     * rPDOCacheManager::CACHE_PHP, который настроен на кэширование в исполняемом формате PHP.
     * @return string Исходный файл карты в строковом формате.
     */
    public function generateObject($obj, $objName, $generateObjVars= false, $generateRelated= false, $objRef= 'this->xpdo', $format= rPDOCacheManager::CACHE_PHP) {
        // $source= false;
        // if (is_object($obj) && $obj instanceof \rPDO\Om\rPDOObject) {
    ...
    public function add($key, & $var, $lifetime= 0, $options= array()) {
        $return= false;
        if ($cache = $this->getCacheProvider($this->getOption(rPDO::OPT_CACHE_KEY, $options))) {
            // $value= null;
            // if (is_object($var) && $var instanceof \rPDO\Om\rPDOObject) {
    ...
    public function replace($key, & $var, $lifetime= 0, $options= array()) {
        $return= false;
        if ($cache = $this->getCacheProvider($this->getOption(rPDO::OPT_CACHE_KEY, $options), $options)) {
            // $value= null;
            // if (is_object($var) && $var instanceof \rPDO\Om\rPDOObject) {
            //     $value= $var->toArray('', true);
            // } else {
    ...
    public function refresh(array $providers = array(), array &$results = array()) {
        ...
        foreach ($providers as $key => $options) {
            if (array_key_exists($key, $this->caches) && !array_key_exists($key, $results)) {
                $results[$key] = $this->clean(array_merge($options, array(rPDO::OPT_CACHE_KEY => $key)));
            }
        }
